...
"""
    ...
    # 检查是否已替换
    if hasattr(expanded, "body"):
        first_stmt = expanded.body[0]
        if hasattr(first_stmt, "expression"):
            expr = first_stmt.expression
            print(f"左操作数: {expr.left}")
            print(f"右操作数: {expr.right}")
            if hasattr(expr.left, "expression"):
                print(f"左操作数.expression: {expr.left.expression}")
            if hasattr(expr.right, "expression"):
                print(f"右操作数.expression: {expr.right.expression}")
